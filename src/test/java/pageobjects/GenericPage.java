...
public class GenericPage {
    ...
    public void switchToAdminFrame(){
        WebElement adminframe = createWaitDriver().until(ExpectedConditions.presenceOfElementLocated(By.xpath("//iframe[contains(@src, 'adminframe')]")));
        getDriver().switchTo().frame(adminframe);
    }
    ...
    protected boolean doesElementPresentOnPage(By locator, int timeout){
        List<WebElement> elements;

        try {
            elements = createWaitDriver(timeout, 300).until(ExpectedConditions.presenceOfAllElementsLocatedBy(locator));
